  ...
  <section class="min-h-screen">
          ...
          <tbody>
            <tr v-for="(el, key) in allLessons" :key="key">
              ...
              <td class="px-6 py-4 space- x-4 space-y-3 text-center text-white">
                ...
                <router-link
                  class="border text-black py-2 px-4 rounded-md"
                  :to="`/Admin/Lessons/${el.id}`"
                >
                  اضافة شرائح
                </router-link>
                <router-link
                  class="border text-black py-2 px-4 rounded-md"
                  :to="`/Admin/Lessons/allSlides/${el.id}`"
                >
                  عرض الشروحات
                </router-link>
              </td>
    ...
    <div class="container md:w-1/2 w-3/4" v-if="!hidden">
      <h1 class="md:text-3xl text-3xl text-center">
        {{ AddNewMood ? "اضافة فصل جديد" : "تعديل " }}
      </h1>
  ...
  methods: {
    async loadLessons() {
      ...
      try {
        ...
        this.allLessons = this.$store.getters["courses/allLessons"].filter(
          (el) => el.courseId == courseId && el.chapterId == chapterid
        );
